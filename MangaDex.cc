...
(function(sites) {
     ...
     * @type {SiteObject}
     */
    sites['www.mangadex.cc'] = {
        preInit: function() {
            //Auto-redirect to non-www version.
            location.href = location.href.replace('www.mangadex.cc', 'mangadex.cc');
        }
    };
    ...
    sites['mangadex.cc'] = {
        preInit: function(callback) {
            ...
            function getPreInitData(callback, _this) {
                ...
                $.getJSON(`${_this.https}://mangadex.cc/api/chapter/${_this.segments[2]}?`, function(chapterData) {
                    preInitData._data = chapterData;
                    $.getJSON(`${_this.https}://mangadex.cc/api/manga/${chapterData.manga_id}?`, function(titleData) {
                        preInitData.manga = {
        ...
        setObjVars: function() {
            ...
            this.title_url = `${_this.https}://mangadex.cc/manga/${titleID}`;
            this.chapter_url = `${_this.https}://mangadex.cc/chapter/${chapter}`;

            let tempList = {};
            titleData.chapterList.forEach((chData) => {
                let chapterNumber = `v${chData.volume}/c${chData.chapter}`.replace(/^v\//, '');
                tempList[`${_this.https}://mangadex.cc/chapter/${chData.id}`] = chapterNumber + (chData.title !== '' ? ' - ' + chData.title : '');
            });
